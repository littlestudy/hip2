...
import org.hip.base.LogReader;

import com.twitter.elephantbird.util.HadoopCompat;

public class ReservoirSamplerInputFormat<K extends Writable, V extends Writable> extends InputFormat<K, V> {
	...
	public static final String USE_SAMPLES_NUMBER_PER_INPUT_SPLIT = "reservoir.smaples.useperinputsplit";
	...
	public static int getNumSamples(Configuration conf){
		...
		boolean userPerSample = conf.getBoolean(USE_SAMPLES_NUMBER_PER_INPUT_SPLIT, DEFAULT_USE_SAMPLES_PER_INPUT_SPLIT);
		if (userPerSample)
			return numSamples;
	...
	public RecordReader createRecordReader(InputSplit split,
		...
		return new ReservoirSampleRecordReader(context,
				getInputFormat(conf).createRecordReader(split, context),
				getNumSamples(conf),
				getMaxRecordsToRead(conf));
	}
	
	public static class ReservoirSampleRecordReader<K extends Writable, V extends Writable>
		...
		public ReservoirSampleRecordReader(TaskAttemptContext context,
											  RecordReader<K, V> rr,
											  int numSamples,
											  int maxRecords) {
			super();
			...
			this.keys = new ArrayList<K>(numSamples);
			this.values = new ArrayList<V>(numSamples);
		}
		...
		public V getCurrentValue() throws IOException, InterruptedException {
			log.showInfo("record reader Value: " + values.get(idx - 1));
